...
class ResultsPlotter:
    ...
    def plot_complete_results(self, filename=None, plot_dates=True, plotting_config=None):
        ...
        self.plot_time_series(fig, gs[current_row, :], probe_data, self.rain_scenario, 
    ...
    def plot_time_series(self, fig, gs_slice, probe_data, rain_scenario, 
    ...
    def _plot_snapshots(self, fig, gs, snapshots, plotting_config, start_row, use_datetime=False):
        ...
        for t in sorted_times:
            if field_name == 'water_table':
                ...
                # Clean the data: remove inf, nan, and extreme outliers
        ...
        if field_name == 'water_table':
            ...
            if len(all_data) > 0:
                vmin_global = max(0.0, np.min(all_data))  # Force non-negative for concentration
                vmax_global = np.max(all_data)
                ...
                # Limit extreme ranges to prevent tick issues
                if vmax_global / vmin_global > 1e6:  # Too wide range
                    vmax_global = vmin_global + 1000  # Reasonable upper limit
            else:
                vmin_global, vmax_global = 0, 1
        ...
        for idx, t in enumerate(sorted_times):
            ...
            if field_name == 'water_table':
                ...
                field_data = snapshots[t][field_name].dat.data[:]
                # Clean outliers for this snapshot
                field_data = np.clip(field_data, vmin_global, vmax_global)
            ...
            # Plot with appropriate levels and colormap - use fewer levels to prevent tick issues
            if field_name == 'water_table':
                levels = np.linspace(0, 1, 11)  # Reduced from 25 to 11
            else:
                # Create smart levels to avoid too many ticks
                if vmax_global - vmin_global > 1000:
                    # For large ranges, use exponential spacing
                    levels = np.logspace(np.log10(max(vmin_global, 1e-6)), 
                                       np.log10(vmax_global), 11)
                else:
                    # For normal ranges, use linear spacing
                    levels = np.linspace(vmin_global, vmax_global, 11)

            cf = ax.contourf(Xi, Yi, Zi, levels=levels,
                            cmap=colormap, vmin=vmin_global, vmax=vmax_global, extend='max')
        ...
        # Format colorbar ticks for large numbers
        if vmax_global > 1000:
            # Use scientific notation for large numbers
            cbar.ax.ticklabel_format(style='scientific', axis='y', scilimits=(0,0))
        elif vmax_global > 100:
            # Use integer formatting for moderate numbers
            cbar.ax.ticklabel_format(style='plain', axis='y')
            ticks = cbar.get_ticks()
            cbar.set_ticks(ticks[::2])  # Use every other tick to reduce clutter
